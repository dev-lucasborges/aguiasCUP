...
const Partida = ({ route }) => {
    ...
    const adicionarAcaoAoHistorico = (teamId, playerId, type, value, playerName) => {
        const novaAcao = { teamId, playerId, type, value, playerName };
        setAcoesHistorico(prevAcoes => [...prevAcoes, novaAcao]);
        exibirMensagemTemporaria(`${type.toUpperCase()} para o jogador: ${playerName}`);
    };

    const initScores = (team, setScore) => {
        const initialScore = {};
        const playersArray = team?.jogadores ? Object.keys(team.jogadores).map(key => ({
            id: key,
            ...team.jogadores[key]
        })) : [];

        playersArray.forEach(player => {
            initialScore[player.id] = { pontos: 0, assistencias: 0, rebotes: 0, faltas: 0 };
        });
    ...
    const renderPlayer = (player, teamId) => {
        return (
            <View style={{justifyContent: "center", alignItems: "center"}}>
                <View style={{flexDirection:"row", justifyContent: "space-around",width:"100%", borderStyle: "solid", borderTopWidth: 1, paddingVertical: 5}}>
                <View style={{width: 200, justifyContent: "center"}}>
                    <Text style={{textTransform: "uppercase"}}>{player.numeroCamisa}. {player.nome}</Text>
                </View>
                    <View style={{flexDirection: "row", width: "50%", justifyContent: "space-around",}}>
                        <TouchableOpacity style={{backgroundColor: "#7BDA85", paddingHorizontal: 10, paddingVertical: 3, borderRadius: 5, }} onPress={() => addScore(teamId, player.id, 'pontos', 1, player.nome)}><Text>P1</Text></TouchableOpacity>
                        <TouchableOpacity style={{backgroundColor: "#7BDA85", paddingHorizontal: 10, paddingVertical: 3, borderRadius: 5, }} onPress={() => addScore(teamId, player.id, 'pontos', 2, player.nome)}><Text>P2</Text></TouchableOpacity>
                        <TouchableOpacity style={{backgroundColor: "#4EB258", paddingHorizontal: 10, paddingVertical: 3, borderRadius: 5, }} onPress={() => addScore(teamId, player.id, 'pontos', 3, player.nome)}><Text>P3</Text></TouchableOpacity>
                        <TouchableOpacity style={{backgroundColor: "#4EA0B2", paddingHorizontal: 10, paddingVertical: 3, borderRadius: 5, }} onPress={() => addScore(teamId, player.id, 'assistencias', 1, player.nome)}><Text>A</Text></TouchableOpacity>
                        <TouchableOpacity style={{backgroundColor: "#E72DF7", paddingHorizontal: 10, paddingVertical: 3, borderRadius: 5, }} onPress={() => addScore(teamId, player.id, 'rebotes', 1, player.nome)}><Text>R</Text></TouchableOpacity>
                        <TouchableOpacity style={{backgroundColor: "#FF4343", paddingHorizontal: 10, paddingVertical: 3, borderRadius: 5, }} onPress={() => addScore(teamId, player.id, 'faltas', 1, player.nome)}><Text>F</Text></TouchableOpacity>
                    </View>
                </View>
            </View>
        );
    ...
    const getTotal = (score, type) => {
        return Object.values(score).reduce((total, playerScore) => total + (playerScore[type] || 0), 0);
    };
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={{ backgroundColor: mapeamentoDeCores[teamA.cor] || "transparent", height: 80, width: "100%", justifyContent:"center", alignItems:"center"}}>
                <View style={{backgroundColor: "#FFF", paddingVertical: 20, paddingHorizontal: 40, borderRadius: 22}}>
                <Text style={{ fontWeight: "bold" }}>
                    {teamA.liga}{' - '}
                    <Text style={{ color: mapeamentoDeCores[teamA.cor], textTransform: 'uppercase' }}>
                        {teamA.cor}
                    </Text>
                </Text>
                </View> 
            ...
            {/* Team B */}
            <View style={[styles.teamContainer]}>
            <View style={{ backgroundColor: mapeamentoDeCores[teamB.cor] || "transparent", height: 80, width: "100%", justifyContent:"center", alignItems:"center"}}>
            <View style={{backgroundColor: "#FFF", paddingVertical: 20, paddingHorizontal: 40, borderRadius: 22}}>
                    <Text style={{ fontWeight: "bold" }}>
                        {teamB.liga}{' - '}
                        <Text style={{ color: mapeamentoDeCores[teamB.cor], textTransform: 'uppercase' }}>
                            {teamB.cor}
                        </Text>
                    </Text>
            </View>
            ...
            {/* Botão Encerrar Jogo */}
            <TouchableOpacity style={styles.encerrarJogoButton} onPress={encerrarPartida}>
                <Text style={styles.encerrarJogoButtonText}>Encerrar Jogo</Text>
            </TouchableOpacity>

            <TouchableOpacity style={styles.reverterAcaoButton} onPress={reverterUltimaAcao}>
                <FontAwesomeIcon style={styles.reverterAcaoTexto}icon={faArrowRotateLeft} />
            </TouchableOpacity>

            {mensagemTemporaria !== "" && (
                <View style={styles.mensagemTemporariaContainer}>
                    <Text style={styles.mensagemTemporariaTexto}>{mensagemTemporaria}</Text>
                </View>
            )}
        </View>
    );
